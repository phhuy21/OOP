...
for t in [
    ...
    'Xử lý các tình huống nghiệp vụ: tương thích máy bay–gate–đường băng, tự động gán cổng mặc định, '
    'kiểm soát xung đột lịch bay của máy bay, quản lý vòng đời chuyến bay qua máy trạng thái.',
...
table(['Lớp', 'Vai trò', 'Kế thừa từ', 'Tính chất OOP'], [
    ...
    ['WideBodyAircraft', 'Thân rộng — 2800m RW, sử dụng cổng hạng 2', 'Aircraft', 'Kế thừa, Đa hình'],
    ['NarrowBodyAircraft', 'Thân hẹp — 2500m RW, sử dụng cổng hạng 1', 'Aircraft', 'Kế thừa, Đa hình'],
    ['TurbopropAircraft', 'Cánh quạt — 1500m RW, sử dụng cổng hạng 0', 'Aircraft', 'Kế thừa, Đa hình'],
...
table(['Lớp', 'Vai trò', 'Tính chất OOP'], [
    ...
    ['Gate', 'Cổng — loại gate + danh sách Reservation đặt chỗ', 'Đóng gói'],
...
for t in [
    ...
    'Điều kiện chuyển sang Ready: phải gán máy bay và cổng khởi hành.',
    'Khi hạ cánh (Landed) và hoàn tất (Completed): giải phóng các tài nguyên đã gán cho chuyến bay.',
]:
    bullet(t)
...
para('Khi mở CSDL, hệ thống đặt các PRAGMA: journal_mode=WAL, foreign_keys=ON, synchronous='
     'NORMAL, busy_timeout=5000ms. Quan hệ nhiều–nhiều (chuyến bay ↔ hành '
...
for t in [
    ...
    'Xử lý 10+ ràng buộc nghiệp vụ phức tạp (tương thích hạ tầng máy bay-đường băng/gate, xung '
    'đột lịch bay của máy bay, kiểm soát hành lý quá mức, quy trình check-in/boarding, hành khách No-show).',
